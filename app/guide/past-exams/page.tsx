      ...
      <main className="pt-32 md:pt-40 pb-20">
        <article className="max-w-4xl mx-auto px-6">
          ...
          <div className="prose prose-slate max-w-none leading-[1.9] text-gray-700">
            ...
            <p className="mb-12">
              SFCの小論文対策を始めたばかりの受験生が、最初に直面する壁です。しかし、完成された模範解答をどれだけ読んでも、本番で同じように書けるようにはなりません。必要なのは、特別なセンスではなく、<strong>「真っ白な原稿用紙から、どのような手順で論理を組み立てていくか」という具体的なプロセス</strong>を身につけることです。本記事では、過去問を解く際の具体的なステップを解説します。
            </p>
            ...
            <p className="mb-6">
              まず行うべきは<strong>「設問文の熟読と条件の抽出」</strong>です。例えば、「資料A〜Dを踏まえ、現在の社会における問題点を〇〇字以内で要約し、それに対するあなたの解決策を〇〇字で提案しなさい」といった具体的な要求をマーキングします。「何について書くべきか」「どの資料を使うよう指定されているか」「字数配分はどうなっているか」という<strong>論理の外枠（制約）</strong>を最初に確定させることが、すべての出発点となります。
            </p>

            <ArticleH2 id="sec2">2. 資料から「真の問題」を見つけ出す思考法</ArticleH2>
            <p>
              設問の要求を把握したら、それに沿って資料を読み解きます。ここで重要なのは、資料に書かれている表面的な事実をそのまま書き写すのではなく、<strong>その背後にある「構造的な問題（ボトルネック）」を見つけ出すこと</strong>です。
            </p>
            <p className="mb-6">
              例えば「食品ロス」に関する資料があったとします。「日本では年間〇〇トンの食品が捨てられている、だからもったいない」と書くのは単なる事実の羅列です。SFCで評価されるのは、「テクノロジーが発展しているのに、なぜ食品の需給予測が最適化されないのか」「法的な規制がリサイクルを阻んでいるのではないか」といった、<strong>問題が解決されない根本的な原因（真の問題）</strong>を提示する力です。佐藤塾では、この「視点の広げ方」をAIとの壁打ちを通じて徹底的にトレーニングします。
            </p>
            ...
            <p className="mb-6">
              必ず、問題用紙の余白を使って<strong>「アウトライン（論理の骨組み）」</strong>を作成してください。箇条書きやマインドマップ形式で、「①真の問題の定義」「②その問題が発生している原因（資料に基づく根拠）」「③原因を取り除くための具体的な解決策」「④解決策がもたらす未来（結論）」というブロックを配置します。この骨組みが論理的に繋がっているかを確認してから、初めて原稿用紙のマス目を埋める作業に入ります。
            </p>

            <ArticleH2 id="sec4">4. SFCが求める「当事者意識（オーナーシップ）」の示し方</ArticleH2>
            <p>
              最後に、解決策を提示する際の最も重要なポイントについてお伝えします。それは<strong>「評論家にならず、当事者（オーナーシップを持つ者）として書く」</strong>ということです。
            </p>
            <p className="mb-12">
              「政府が法律を変えるべきだ」「国民の意識を高めるべきだ」といった、誰かがやってくれるのを待つような意見は評価されません。<strong>「自分自身が（あるいは自分が立ち上げるプロジェクトが）、既存のテクノロジーや社会の仕組みをどのように活用し、この問題を解決に導くのか」</strong>という、地に足のついた実現可能性と当事者意識が求められます。佐藤塾の1on1指導では、生徒一人ひとりの興味関心とSFCの出題テーマをすり合わせ、この「自分事として語る力」を磨き上げていきます。
            </p>
